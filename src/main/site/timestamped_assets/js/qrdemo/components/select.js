// Code from https://github.com/bluzky/nice-select2
...
export function getSelect(label, description) {
    ...
    div.append(labelElement, descriptionElement)
    option.append(div)
    option.dataset.extra = description
    return option
}
...
NiceSelect.prototype.renderDropdown = function () {
    ...
    html += `<div class="${this.multiple ? "multiple-options" : "current"}"></div>`;
...
NiceSelect.prototype._renderSelectedItems = function () {
    if (this.multiple) {
        ...
        this.dropdown.querySelector(".multiple-options").innerText = selectedHtml;
    } else {
        var text = this.selectedOptions.length > 0 ? this.selectedOptions[0].data.text : this.placeholder;

        let element = this.dropdown.querySelector(".current")
        element.innerHTML = ''
