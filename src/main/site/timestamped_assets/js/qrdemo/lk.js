...
function setSettings(div, settingsJson, userDetails) {
    ...
    if (!admin) {
        ...
        let hint = getTextLabel('Используйте {{qr-code}} - для номера карточки и {{qr-link}} - для ссылки на текущую карточку в тексте для печатной формы')
...
function setRequestPanel(parenDiv, registrationId) {
    ...
    adminApi().getRegistrationRequest(registrationId)
        ...
        .then(json => {
            ...
            let firstName = get2TextLabels('Контактное лицо: ', json?.firstName)
            let lastName = get2TextLabels('Фамилия: ', json?.lastName)
            let organization = get2TextLabels('Организация: ', json?.organization)
            let phoneNumber = get2TextLabels('Номер телефона: ', json?.phoneNumber)
            let country = get2TextLabels('Страна: ', json?.country)
            let city = get2TextLabels('ИНН: ', json?.city)
            let userAgent = get2TextLabels('Браузер: ', json?.userAgent)
            let website = get2TextLabels('Ссылка на сайт: ', json?.webSite)
            ...
            parenDiv.appendChild(document.createElement('br'))
            parenDiv.appendChild(organization)
            parenDiv.appendChild(firstName)
            parenDiv.appendChild(email)
            parenDiv.appendChild(phoneNumber)
            parenDiv.appendChild(city)
            parenDiv.appendChild(website)
            parenDiv.appendChild(document.createElement('br'))
